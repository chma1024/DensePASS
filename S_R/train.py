...
ITER_SIZE = 1
NUM_WORKERS = 2
DATA_DIRECTORY = 'C:/Users/eggpa/datasets/Cityscapes'
IGNORE_LABEL = 19
INPUT_SIZE = '1280,720'
...
def get_arguments():
    ...
    parser.add_argument("--tensorboard", action='store_true', default=False, help="choose whether to use tensorboard.")
...
def main():
    ...
    if args.model == 'DeepLab':
            ...
            for name, param in state_dict.items():
                if name not in own_state:
                    if name.startswith("module."):
                        ...
                        if name.split("module.")[-1].startswith("clslayer."):
                            own_state["clslayer1." + name.split("module.")[-1].split("clslayer.")[-1]].copy_(param)
                    else:
                        print(name, " not loaded")
                        continue
                else:
        ...
        model = load_my_state_dict(model, torch.load(args.restore_from, map_location=lambda storage, loc: storage))
    elif args.model == 'RERFNet':
        model = RERFNet(args.num_classes)
        def load_my_state_dict(model, state_dict):  #custom function to load model when not all dict elements
            own_state = model.state_dict()
            for name, param in state_dict.items():
                if name not in own_state:
    ...
    elif args.model == 'ERFNet' or args.model == 'FANet' or args.model == 'DANet' or args.model == 'RDANet' or args.model == 'RERFNet' or args.model == 'RFANet':
    ...
    seg_loss = CrossEntropyLoss2d(weight) #torch.nn.CrossEntropyLoss(ignore_index=19)#args.ignore_label)
    ...
    for i_iter in range(args.num_steps):
        ...
        print(
        'iter = {0:8d}/{1:8d}, loss_seg1 = {2:.3f} loss_seg2 = {3:.3f} loss_adv1 = {4:.3f}, loss_adv2 = {5:.3f} loss_D1 = {6:.3f} loss_D2 = {7:.3f} cur_best = {8:.3f}% b_best = {9:.3f}%'.format(
            i_iter, args.num_steps, loss_seg_value1, loss_seg_value2, loss_adv_target_value1, loss_adv_target_value2, loss_D_value1, loss_D_value2, best * 100, b_best * 100))
